...
class SplayNode(BinaryNode):
    ...
    def splay(self, node):
        if self != None and _Key(node.key) != _Key(self.key):
            if self.left != None:
                if _Key(self.left.key) == _Key(node.key):  # node ist links unter der wurzel
                    self._zig_rot(self.left)
                elif self.left.right != None and _Key(self.left.right.key) == _Key(node.key):
                    # erst rechts dann links
                    self._zag_zig_rot(self.left.right)
                elif self.left.left != None and _Key(self.left.left.key) == _Key(node.key):
            ...
            if self.right != None:
                ...
                elif self.right.left != None and _Key(self.right.left.key) == _Key(node.key):
                    self._zig_zag_rot(self.right.left)
                elif self.right.right != None and _Key(self.right.right.key) == _Key(node.key):
                    self._zag_zag_rot(self.right.right)
            if self.left != None and _Key(node.key) < _Key(self.key):
